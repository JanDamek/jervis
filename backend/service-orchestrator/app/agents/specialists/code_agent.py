...
class CodingAgent(BaseAgent):
    ...
    async def execute(
        ...
        system_prompt = (
            ...
            "- claude: Best all-round agent — reasoning, architecture, complex code changes, documentation\n"
            "- kilo: Free agent using aider + OpenRouter free models — use for simple tasks "
            "(bug fixes, small features, code formatting, simple refactoring). "
            "Prefer kilo when: task is straightforward, client has no paid tier, "
            "or task explicitly requests kilo. "
            "Use claude when: task needs complex reasoning, architecture decisions, "
            "multi-file refactoring, or kilo previously failed on the same task.\n\n"
            "Workflow:\n"

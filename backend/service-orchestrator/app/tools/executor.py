...
async def _get_workspace_path(
    ...
    Kotlin server's ServerGitService.InitWorkspace gRPC (blocking) which
    ...
    try:
        ...
        logger.warning(
            ...
            project_id, status, resp.error or "",
        )
        return None
    except Exception as e:
...
def _env_stub_imports():
    from app.grpc_server_client import server_environment_stub
    from jervis.common import types_pb2
    ...
    ctx = types_pb2.RequestContext()
    prepare_context(ctx)
    return server_environment_stub(), environment_pb2, ctx
...
async def _execute_environment_get(environment_id: str) -> str:
    ...
    try:
        stub, environment_pb2, ctx = _env_stub_imports()
        resp = await stub.GetEnvironment(
...
async def _execute_environment_create(
    ...
    try:
        stub, environment_pb2, ctx = _env_stub_imports()
        resp = await stub.CreateEnvironment(
...
async def _execute_environment_add_component(
    ...
    if env_vars:
        try:
            env_vars_map = _json.loads(env_vars)
        except Exception:
            return f"Error: Invalid JSON for env_vars: {env_vars}"
    try:
        stub, environment_pb2, ctx = _env_stub_imports()
        resp = await stub.AddComponent(
...
async def _execute_environment_deploy(environment_id: str) -> str:
    ...
    try:
        stub, environment_pb2, ctx = _env_stub_imports()
        resp = await stub.DeployEnvironment(
...
async def _execute_environment_status(environment_id: str) -> str:
    ...
    try:
        stub, environment_pb2, ctx = _env_stub_imports()
        resp = await stub.GetEnvironmentStatus(
            environment_pb2.EnvironmentIdRequest(ctx=ctx, environment_id=environment_id),
            timeout=30.0,
        )
        status = _json.loads(resp.body_json)
...
async def _execute_environment_sync(environment_id: str) -> str:
    ...
    try:
        stub, environment_pb2, ctx = _env_stub_imports()
        resp = await stub.SyncEnvironment(
            environment_pb2.EnvironmentIdRequest(ctx=ctx, environment_id=environment_id),
            timeout=60.0,
        )
        env = MessageToDict(resp, preserving_proto_field_name=False)
...
async def _execute_environment_clone(
    ...
    try:
        stub, environment_pb2, ctx = _env_stub_imports()
        resp = await stub.CloneEnvironment(
...
async def _execute_environment_add_property_mapping(
    ...
    try:
        stub, environment_pb2, ctx = _env_stub_imports()
        resp = await stub.AddPropertyMapping(
        ...
        return f"Error adding property mapping: {msg[:300]}"
...
async def _execute_environment_auto_suggest_mappings(environment_id: str) -> str:
    ...
    try:
        stub, environment_pb2, ctx = _env_stub_imports()
        resp = await stub.AutoSuggestPropertyMappings(
            environment_pb2.EnvironmentIdRequest(ctx=ctx, environment_id=environment_id),
            timeout=30.0,
        )
        env = MessageToDict(resp, preserving_proto_field_name=False)
...
async def _execute_create_git_repository(
    ...
    """Create a git repository via ServerGitService.CreateRepository (gRPC)."""
    if not client_id or not name:
        return "Error: client_id and name are required."
    try:
        import json as _json
        ...
        try:
            data = _json.loads(resp.body_json or "{}")
        except Exception:
            data = {}
        return (
...
async def _execute_init_workspace(project_id: str) -> str:
    """Trigger workspace initialization via ServerGitService.InitWorkspace (gRPC)."""
    if not project_id:
        return "Error: project_id is required."
    try:
        from app.grpc_server_client import server_git_stub
...
# ============================================================
# O365 tools (via O365 Gateway — gRPC passthrough)
# ============================================================

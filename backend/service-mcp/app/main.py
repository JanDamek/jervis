...
@mcp.tool
async def environment_upload_file(
    environment_id: str,
    component_name: str,
    ...
    """Upload a file to a running component pod (e.g., SQL dump, seed data).

    The file will be placed in the target directory inside the pod.
    Use this before executing import commands.

    Args:
        environment_id: Environment ID
        component_name: Name of the component (must be running)
        file_content_base64: File content encoded as Base64
        file_name: Original file name (e.g., "dump.sql", "seed.json")
        target_dir: Target directory in the pod (default: /tmp)
    """
    base = f"{settings.kotlin_server_url}/internal/environments"
    import io

    # Use multipart upload
    async with httpx.AsyncClient(timeout=120) as client:
        files = {"file": (file_name, io.BytesIO(base64_decode(file_content_base64)), "application/octet-stream")}
        data = {"targetDir": target_dir, "fileName": file_name}
        resp = await client.post(
            f"{base}/{environment_id}/components/{component_name}/upload",
            files=files,
            data=data,
        )
        if resp.status_code != 200:
        ...
        return f"File uploaded: {result.get('targetPath', '?')} ({result.get('sizeBytes', 0)} bytes)"


@mcp.tool
async def environment_exec(
    environment_id: str,
    component_name: str,
    command: str,
) -> str:
    ...
    Use after uploading files to run import commands, e.g.:
    - PostgreSQL: "psql -U postgres -f /tmp/dump.sql"
    - MongoDB: "mongorestore --dir /tmp/dump"
    - MySQL: "mysql -u root -p$MYSQL_ROOT_PASSWORD < /tmp/dump.sql"
    - Redis: "redis-cli < /tmp/commands.txt"

    Args:
        environment_id: Environment ID
        component_name: Name of the component (must be running)
        command: Shell command to execute (will be run via sh -c)
    """
    base = f"{settings.kotlin_server_url}/internal/environments"
    async with httpx.AsyncClient(timeout=120) as client:
        resp = await client.post(
            f"{base}/{environment_id}/components/{component_name}/exec",
...
@mcp.tool
async def environment_list_files(
    environment_id: str,
    component_name: str,
    directory: str = "/tmp",
) -> str:
    """List files in a directory inside a running component pod.

    Args:
        environment_id: Environment ID
        component_name: Name of the component
        directory: Directory path to list (default: /tmp)
    """
    base = f"{settings.kotlin_server_url}/internal/environments"
    async with httpx.AsyncClient(timeout=30) as client:
        resp = await client.get(
            f"{base}/{environment_id}/components/{component_name}/files",

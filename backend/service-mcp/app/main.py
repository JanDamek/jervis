...
async def o365_mail_read(
    ...
    body_content = m.body.content[:3000] if m.HasField("body") else ""
    parts = [
...
async def o365_files_list(
    ...
    for item in items:
        ...
        child_count = ""
        if is_folder and item.folder.child_count:
            child_count = f" [{item.folder.child_count} items]"
        lines.append(
            f"{icon} {item.name or '?'}{size}{child_count} "

...
class GoalExecutor(
    ...
    companion object {
        ...
        // Convert model TaskDocument to entity TaskDocument for agents
        private fun toEntity(model: ModelTaskDocument): EntityTaskDocument {
            return EntityTaskDocument(
    ...
    /**
     * Execute single goal completely
     */
    suspend fun executeGoal(
        ...
        try {
            ...
            // Phase 3: Execution
            val evidence = executePlan(goal, plan, entityTask, task, toolRegistry)

            // Phase 4: Review (for complex goals)
            val review = if (shouldReview(goal)) {
    ...
    private suspend fun gatherContext(
        ...
        val enrichedQuery = if (previousResults.isNotEmpty()) {
            buildString {
    ...
    private suspend fun executePlan(
        ...
        val prompt = Prompt.build("goal-executor-${goal.id}") {
            system("""
...
EXECUTION RULES:
1. For each step in the plan, use corresponding tools from the available registry
2. ALWAYS actually call the tools - don't just describe what you would do!
...
- Cloud coding services and APIs = PAID (use only when necessary)
        ...
        val executionStrategy = strategy<String, String>("Execute Goal ${goal.id}") {
    ...
    private fun shouldReview(goal: GoalSpec): Boolean =
        goal.estimatedComplexity == "HIGH" || goal.type == RequestType.CODE_CHANGE
    ...
    private suspend fun reviewExecution(
        ...
        return reviewerAgent.run(
            ...
            evidence = evidence,
            currentIteration = 0,
            maxIterations = 1  // Single review for sub-goals
        )
    }
    ...
    private fun extractArtifacts(evidence: EvidencePack?): List<Artifact> {
        ...
        // Extract structured artifacts from evidence
        // In future: parse for Jira IDs, file paths, email IDs, etc.
        return emptyList()
    }
}

...
data class OrchestratorState(
    ...
    fun withInterpretation(interpretation: NormalizedRequest) = copy(interpretation = interpretation)

    fun withContext(context: ContextPack) = copy(context = context)

    fun withPlan(plan: OrderedPlan) = copy(plan = plan, currentStepIndex = 0)

    fun withEvidence(evidence: EvidencePack) = copy(evidence = evidence)

    fun withReview(review: ReviewResult) = copy(review = review)

    fun withStepExecuted(step: PlanStep) =
        copy(
            executedSteps = executedSteps + step,
            currentStepIndex = currentStepIndex + 1,
        )

    fun withFinalAnswer(answer: String) = copy(finalAnswer = answer)

    fun withUserQuestion(
        question: String,

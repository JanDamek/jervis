...
@LLMDescription(
    "Complex user request decomposed into multiple executable goals with dependency tracking. Use this when a single query requires multiple distinct actions or workflows.",
)
data class MultiGoalRequest(
     ...
     */
    fun hasMoreGoals(completedGoalIds: Set<String>): Boolean = goals.any { it.id !in completedGoalIds }
}
...
data class GoalSpec(
    ...
    @property:LLMDescription(
        "Goal type: ADVICE (research/info), CODE_ANALYSIS (read-only code exploration), CODE_CHANGE (actual modification), MESSAGE_DRAFT, EPIC, or BACKLOG_PROGRAM",
    )
...
sealed class GoalSelection {
    data class Execute(
        val goal: GoalSpec,
        val remaining: MultiGoalRequest,
    ) : GoalSelection()

    data class AllDone(
        val request: MultiGoalRequest,
        val results: List<GoalResult>,
    ) : GoalSelection()

    data class WaitingForDependencies(
        val request: MultiGoalRequest,
    ) : GoalSelection()
}

 ...
 */
class ContentClassificationTools(
    ...
    suspend fun classifyJira(
        ...
        val classificationEnum =
            when (classification.uppercase()) {
                "INFO" -> JiraClassification.INFO
                "ACTIONABLE" -> JiraClassification.ACTIONABLE
                "CUSTOM_GROUP" -> JiraClassification.CUSTOM_GROUP
                else -> {
                ...
                try {
                    ...
                    val taskDescription =
                        buildString {
                            ...
                            append("\nReason: $reason\n")
                            append("\nOriginal content: ${task.content.take(500)}")
                        }

                    userTaskService.createTask(
                        title = taskTitle,
                        description = taskDescription,
                        priority = if (classificationEnum == JiraClassification.ACTIONABLE) TaskPriorityEnum.HIGH else TaskPriorityEnum.MEDIUM,

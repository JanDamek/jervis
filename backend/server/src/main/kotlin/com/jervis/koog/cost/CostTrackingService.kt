...
class CostTrackingService(
    ...
    suspend fun checkBudget(projectId: ProjectId, modelId: String, inputTokens: Int): Boolean {
        ...
        if (!llmPriceService.hasBudget(policy.monthlyBudgetLimit, monthlySpent, estimatedCost)) {
            logger.warn { "BUDGET_EXCEEDED | projectId=$projectId | limit=${policy.monthlyBudgetLimit} | spent=$monthlySpent | estimated=$estimatedCost" }
            return false
        }
    ...
    suspend fun getMonthlySpent(projectId: ProjectId): Double {
        val firstDayOfMonth = LocalDate.now().withDayOfMonth(1).atStartOfDay(ZoneId.systemDefault()).toInstant()

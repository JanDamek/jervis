...
class EnvironmentK8sService(
    ...
    /**
     * Upload a file into a running pod for a specific component.
     * Finds the pod by label `app=componentName`, then uses fabric8 exec to write
     * the file into the specified target directory inside the container.
     *
     * @param namespace K8s namespace
     * @param componentName component name (matches deployment/pod label)
     * @param fileBytes raw file content
     * @param fileName original file name
     * @param targetDir target directory inside the pod (e.g., "/tmp" or "/docker-entrypoint-initdb.d")
     * @return the full path of the file inside the pod
     */
    fun uploadFileToPod(
        namespace: String,
        ...
        buildK8sClient().use { client ->
            ...
            val targetPath = "$targetDir/$sanitizedFileName"

            logger.info { "Uploading file '$sanitizedFileName' (${fileBytes.size} bytes) to pod $podName:$targetPath" }

            // Ensure target directory exists
            try {
                val mkdirExec = client.pods()
                    .inNamespace(namespace)
                    .withName(podName)
                    .redirectingInput()
                ...
                logger.warn { "Could not ensure directory $targetDir in pod $podName: ${e.message}" }
            }

            // Write file via base64 decode to avoid binary stream issues
            val base64Content = java.util.Base64.getEncoder().encodeToString(fileBytes)
            // Split into chunks for large files (shell arg length limit)
            val chunkSize = 65_000
            if (base64Content.length <= chunkSize) {
                val execWatch = client.pods()
                    .inNamespace(namespace)
                    .withName(podName)
                    .redirectingInput()
                    .exec("sh", "-c", "echo '$base64Content' | base64 -d > $targetPath")
                execWatch.close()
                Thread.sleep(500)
            } else {
                // For large files: stream via stdin
                val execWatch = client.pods()
                    .inNamespace(namespace)
                    .withName(podName)
                    .redirectingInput()
                    .exec("sh", "-c", "base64 -d > $targetPath")
                try {
                    // Write base64 in chunks
                    val inputBytes = base64Content.toByteArray(Charsets.US_ASCII)
                    var offset = 0
                    while (offset < inputBytes.size) {
                        val end = minOf(offset + 8192, inputBytes.size)
                        execWatch.input.write(inputBytes, offset, end - offset)
                        execWatch.input.flush()
                        offset = end
                    }
                    execWatch.input.write('\n'.code)
                    execWatch.input.flush()
                    execWatch.input.close()
                    Thread.sleep(1000)
                } finally {
                    execWatch.close()
                }
            ...
            logger.info { "File uploaded to $podName:$targetPath (${fileBytes.size} bytes)" }
            return targetPath
        }
    }

    /**
     * Execute a command inside a running pod for a specific component.
     * Used after file upload to run import commands (e.g., psql -f, mongorestore).
     *
     * @return stdout + stderr output (truncated to 10KB)
     */
    fun execInPod(
        namespace: String,
        componentName: String,
        ...
        buildK8sClient().use { client ->
            val podName = findRunningPod(client, namespace, componentName)
            logger.info { "Executing in pod $podName: ${command.joinToString(" ")}" }
            ...
            try {
                Thread.sleep(3000) // Give command time to execute
                val output = outputStream.toString(Charsets.UTF_8)
                val error = errorStream.toString(Charsets.UTF_8)
                val result = if (error.isNotBlank()) "$output\n[stderr]: $error" else output
                return result.take(10_000)
            } finally {
                execWatch.close()
            }
        }
    ...
    /**
     * List files in a directory inside a running pod.
     */
    fun listFilesInPod(
        namespace: String,
        componentName: String,
        ...
        buildK8sClient().use { client ->
            ...
            try {
                Thread.sleep(1000)
                return outputStream.toString(Charsets.UTF_8).take(5_000)
            } finally {
                execWatch.close()
            }
        }
    ...
    private fun findRunningPod(client: KubernetesClient, namespace: String, componentName: String): String {
        val pods = client.pods()
            .inNamespace(namespace)
            .withLabel("app", componentName)
            .list()
            .items
        ...
        if (pods.isEmpty()) {
            throw IllegalStateException(
                "No running pod found for component '$componentName' in namespace '$namespace'. " +
                    "Ensure the environment is provisioned and the component is running."
            )
        }
        return pods.first().metadata.name
    }

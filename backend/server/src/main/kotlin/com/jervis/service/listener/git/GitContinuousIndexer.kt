...
class GitContinuousIndexer(
    ...
    override suspend fun processAndIndex(
        ...
        try {
            ...
            val metadataResult =
                metadataIndexer.indexGitHistory(
                    project = account,
                    projectPath = gitDir,
            ...
            val codeResult =
                codeIndexer.indexCommitCodeChanges(
                    project = account,
                    projectPath = gitDir,
            ...
            if (!success) {
                logger.warn {
                    "Indexing produced 0 chunks for commit ${commit.commitHash.take(
                        8,
                    )} - metadata: ${metadataResult.processedCommits}, code: ${codeResult.indexedFiles}"
                }
            }

 ...
 * Package-private - only accessible within com.jervis.rag.
 *
 * NOTE: Schema initialization and migration handled by WeaviateSchemaInitializer @PostConstruct.
 * This component assumes schema is already ready when it starts.
 */
@Component
    ...
    private fun buildWhereFilter(filters: VectorFilters): WhereFilter? {
        ...
        conditions.add(
            WhereFilter
                ...
                .valueText(filters.clientId.toHexString())
                .build(),
        )
        ...
        filters.projectId?.let {
            conditions.add(
                WhereFilter
                    ...
                    .path("projectId")
                    .operator(Operator.Equal)
                    .valueText(it.toHexString())
                    .build(),
            )
        }

        // Knowledge types
        filters.knowledgeTypes?.takeIf { it.isNotEmpty() }?.let { types ->
            val typeFilters =
                types.map { type ->
                    WhereFilter
                        .builder()
            ...
            if (typeFilters.size == 1) {
                conditions.add(typeFilters.first())
            } else {
                conditions.add(
                    WhereFilter
                        .builder()
                        .operator(Operator.Or)
                        .operands(*typeFilters.toTypedArray())
                        .build(),
                )
            }
        }

...
class WeaviatePerClientProvisioner(
    ...
    fun ensureClientCollections(clientId: ObjectId) {
        val textClassName = WeaviateClassNameUtil.textClassFor(clientId)
        val codeClassName = WeaviateClassNameUtil.codeClassFor(clientId)
        val client = client()

        ensureClass(client, textClassName)
        ensureClass(client, codeClassName)
    }

    fun health(clientId: String): Boolean {
        return runCatching {
            val schema =
                client()
                    .schema()
            ...
            val text = classes.any { it.className == WeaviateClassNameUtil.textClassFor(clientId) }
            val code = classes.any { it.className == WeaviateClassNameUtil.codeClassFor(clientId) }
            text && code
        }.getOrDefault(false)
    }

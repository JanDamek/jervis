...
import com.jervis.infrastructure.oauth2.OAuth2Provider
...
class OAuth2Service(
    ...
    private val oauth2Properties: com.jervis.infrastructure.config.properties.OAuth2Properties,
) {
    ...
    private fun getProviderConfig(provider: OAuth2Provider): com.jervis.infrastructure.config.properties.OAuth2ProviderConfig =
        when (provider) {
     ...
     *
     * GitHub Apps issue user-to-server tokens that expire after 8 hours (with refresh_token).
     * Classic GitHub OAuth Apps issue non-expiring tokens (no refresh_token) — skipped.
     *
    ...
    suspend fun refreshAccessToken(connection: ConnectionDocument, force: Boolean = false): Boolean {
        ...
        // No refresh token → cannot refresh (classic GitHub OAuth, or missing token)
        if (connection.refreshToken == null) {
            if (force && provider != OAuth2Provider.GITHUB) {

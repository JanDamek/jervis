...
class RpcClientsConfig(
    ...
    fun rpcReconnectHandler(): RpcReconnectHandler =
        object : RpcReconnectHandler {
            override suspend fun reconnectKnowledgebase() {
                _knowledgeService = KnowledgeServiceRestClient(endpoints.knowledgebase.baseUrl, kbCallbackBaseUrl, kbMaintenanceGrpc)
            }
        }
    ...
    private fun getKnowledgeService(): KnowledgeServiceRestClient =
        _knowledgeService ?: synchronized(this) {
            _knowledgeService
                ?: KnowledgeServiceRestClient(kbWriteUrl(), kbCallbackBaseUrl, kbMaintenanceGrpc).also { _knowledgeService = it }
        }

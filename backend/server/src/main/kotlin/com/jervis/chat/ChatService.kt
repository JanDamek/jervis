...
class ChatService(
    ...
    suspend fun getHistory(
        ...
        val backgroundCount = chatMessageService.countByRole(session.id, MessageRole.BACKGROUND).toInt()
    ...
    suspend fun getBackgroundHistory(
        ...
        val messages = if (beforeMessageId != null) {
            chatMessageService.getMessagesByRoleBefore(session.id, MessageRole.BACKGROUND, ObjectId(beforeMessageId), limit)
        } else {
            chatMessageService.getMessagesByRole(session.id, MessageRole.BACKGROUND, limit)
        }

        val totalCount = chatMessageService.countByRole(session.id, MessageRole.BACKGROUND)
        val hasMore = messages.size == limit && messages.size.toLong() < totalCount

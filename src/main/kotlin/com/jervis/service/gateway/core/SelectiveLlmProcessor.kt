...
class SelectiveLlmProcessor(
     ...
     * @param userPrompt The user prompt to chunk if needed
     * @param responseSchema The expected response schema
     * @param quick Whether to use quick processing
    ...
    private fun chunkUserPrompt(
        ...
        if (availableTokensPerChunk <= 0) {
            ...
            return listOf(userPrompt) // Fallback to single chunk
        }
    ...
    private suspend fun <T : Any> processChunks(
        ...
        type: PromptTypeEnum,
        executor: suspend (systemPrompt: String, userPrompt: String) -> T,
    ): SelectiveProcessingResult<T> = coroutineScope {

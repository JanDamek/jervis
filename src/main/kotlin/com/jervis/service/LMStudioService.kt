...
class LMStudioService {
    ...
    fun testConnection(url: String): Boolean {
        try {
            ...
            restTemplate.exchange("$baseUrl/models", HttpMethod.GET, entity, ModelsResponse::class.java)
    ...
    fun getAvailableModels(url: String): List<LMStudioModel> {
        ...
        val now = Instant.now()
        if (Duration.between(lastModelsCheck, now).seconds < 60 && lastModelsCheck != Instant.EPOCH && cachedModels.isNotEmpty()) {
            return cachedModels
        }
        ...
        try {
            ...
            val response = restTemplate.exchange("$baseUrl/models", HttpMethod.GET, entity, ModelsResponse::class.java)

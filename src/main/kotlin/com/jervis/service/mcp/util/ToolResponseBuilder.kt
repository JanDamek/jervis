 ...
 * 
 * Ensures consistent format across all tools with:
...
object ToolResponseBuilder {
     ...
     * @param summary Concise summary of what was accomplished (max 150 chars)
     ...
     */
    fun success(toolName: String, summary: String, content: String = ""): String =
        buildResponse(toolName, summary, content)

    /**
     * Creates a response with metadata section.
     */
    fun withMetadata(
        toolName: String, 
        summary: String, 
        content: String, 
        metadata: String
    ): String = buildResponse(toolName, summary, content, metadata)
    ...
    fun fileResult(
        ...
        val summary = "$action $fileName"
        return if (details.isNotBlank()) {
            buildResponse(toolName, summary, details, content)
        } else {
            buildResponse(toolName, summary, content)
        }
    }
    ...
    /**
     * Creates a response for search/query results.
     */
    fun searchResult(
        toolName: String,
        resultCount: Int,
        ...
        val summary = "Found $resultCount result${if (resultCount != 1) "s" else ""}"
        val content = buildString {
            if (queryInfo.isNotBlank()) {
                appendLine(queryInfo)
                appendLine()
            }
            append(results)
        }
    ...
    fun listingResult(
        ...
        val summary = "Listed $itemCount $itemType"
        val content = buildString {

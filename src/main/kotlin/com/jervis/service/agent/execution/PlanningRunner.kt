...
class PlanningRunner(
    ...
    suspend fun run(taskContext: TaskContext): Boolean {
        logger.info { "AGENT_LOOP_PLAN_START: Planning loop for context: ${taskContext.id}" }
        logger.debug {
        ...
        logger.info { "AGENT_LOOP_PLAN_END: Final resolution status = ${resolutionResult.complete}" }
        logger.debug {
    ...
    private suspend fun createAdditionalPlan(
        ...
        val mockOriginalPlan =
            Plan(
                ...
                investigationGuidance = emptyList(),

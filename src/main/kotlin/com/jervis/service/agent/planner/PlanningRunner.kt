...
class PlanningRunner(
    ...
    suspend fun run(taskContext: TaskContext): Boolean {
        ...
        val resolutionResult = taskResolutionChecker.checkIfIsResolvDetailed(taskContext)
        logger.info { 
            "PLANNING_RUNNER_RESOLUTION_CHECK: isComplete=${resolutionResult.isComplete}, " +
            "plans=${resolutionResult.completedPlans}/${resolutionResult.totalPlans}, " +
            "steps=${resolutionResult.completedSteps}/${resolutionResult.totalSteps}, " +
            "missingItems=${resolutionResult.missingItems.size}"
        }

        if (!resolutionResult.isComplete && resolutionResult.missingItems.isNotEmpty()) {
            ...
            try {
                ...
                taskContext.plans = taskContext.plans + additionalPlan
                taskContextService.save(taskContext)
                
                logger.info { 
                    "PLANNING_RUNNER_ADDITIONAL_PLAN_CREATED: planId=${additionalPlan.id}, " +
                    "steps=${additionalPlan.steps.size}, addressing ${resolutionResult.missingItems.size} missing items"
                }
                ...
                // Continue with original result even if re-planning fails
            }
        }

        logger.info { "AGENT_LOOP_END: Final resolution status = ${resolutionResult.isComplete}" }
        logger.debug {
            "PLANNING_RUNNER_FINAL_PLANS: ${taskContext.plans.map { "planId=${it.id}, status=${it.status}, steps=${it.steps.size}" }}"
        }
        
        return resolutionResult.isComplete
    }

    /**
     * Creates a comprehensive prompt describing missing requirements for re-planning
     */
    private fun buildMissingRequirementsPrompt(resolutionResult: TaskResolutionChecker.ResolutionResult): String {
        return buildString {
            appendLine("The following requirements are missing or incomplete and need to be addressed:")
            appendLine()
            resolutionResult.missingItems.forEachIndexed { index, item ->
     ...
     */
    private suspend fun createAdditionalPlan(taskContext: TaskContext, missingRequirementsDescription: String): Plan {
        // Create a mock original plan context for the planner
        val mockOriginalPlan = Plan(
            id = ObjectId(),

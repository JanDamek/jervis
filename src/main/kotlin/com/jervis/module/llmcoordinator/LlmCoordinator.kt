...
class LlmCoordinator(
    ...
    init {
        ...
        if (settingService.getStringValue(SettingService.SYSTEM_PROMPT_CODE) == "none") {
            settingService.saveValue(SettingService.SYSTEM_PROMPT_CODE, 
        ...
        if (settingService.getStringValue(SettingService.SYSTEM_PROMPT_EXPLANATION) == "none") {
            settingService.saveValue(SettingService.SYSTEM_PROMPT_EXPLANATION, 
        ...
        if (settingService.getStringValue(SettingService.SYSTEM_PROMPT_SUMMARY) == "none") {
            settingService.saveValue(SettingService.SYSTEM_PROMPT_SUMMARY, 
        ...
        if (settingService.getStringValue(SettingService.SYSTEM_PROMPT_GENERAL) == "none") {
            settingService.saveValue(SettingService.SYSTEM_PROMPT_GENERAL, 
                "You are a helpful assistant. Provide accurate and relevant information.")
        }

        // Default LLM models for Anthropic
     ...
     */
    fun processQuery(query: String, context: String, options: Map<String, Any> = emptyMap()): LlmResponse {
        ...
        try {
            ...
            if (llmProvider.startsWith("anthropic.")) {
            ...
            } else if (llmProvider.startsWith("openai.")) {
                ...
                return LlmResponse(
                    answer = response.choices.firstOrNull()?.message?.content ?: "No response from LLM",
                    model = response.model,
            ...
            if (e.message?.contains("rate limit exceeded", ignoreCase = true) == true && llmProvider.startsWith("anthropic.")) {
                // Log the fallback
                println("Anthropic rate limit exceeded, falling back to OpenAI")

                // Select the appropriate OpenAI model based on query type
                val openAiModel = when (queryType) {
    ...
    private fun callAnthropicApi(
        ...
        val response = restTemplate.postForObject(apiUrl, entity, AnthropicResponse::class.java)
     ...
     */
    private fun selectLlmProvider(queryType: QueryType, options: Map<String, Any>): String {
     ...
     */
    private fun estimateTokenCount(systemPrompt: String, userMessage: String): Int {
...
enum class QueryType {
    CODE, EXPLANATION, SUMMARY, GENERAL
}

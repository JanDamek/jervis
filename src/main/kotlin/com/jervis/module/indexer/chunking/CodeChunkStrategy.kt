...
class CodeChunkStrategy(
    ...
    private val codeLanguages = setOf(
        ...
        // Scripting languages
        "py", "rb", "php", "sh", "bash", "zsh", "ps1", "bat", "cmd",
        // Systems languages
        "c", "cpp", "h", "hpp", "cs", "go", "rs", "swift",
        // Data formats
        "json", "yaml", "yml", "xml", "toml", "ini", "properties", "sql"
    )
    ...
    override fun splitContent(
        ...
        return when (language.lowercase()) {
            "kt", "kts" -> chunkKotlinCode(content, tokenizer, actualMaxChunkSize, actualOverlapSize)
            "java" -> chunkJavaCode(content, tokenizer, actualMaxChunkSize, actualOverlapSize)
            else -> chunkGenericCode(content, language, tokenizer, actualMaxChunkSize, actualOverlapSize)
        }
    }
    
    override fun canHandle(contentType: String): Boolean {
        return contentType.lowercase() in codeLanguages
    }
    ...
    private fun chunkKotlinCode(
        ...
        val packagePattern = Pattern.compile("^\\s*package\\s+(\\S+)")
        val importPattern = Pattern.compile("^\\s*import\\s+(\\S+)")
        ...
        val headerEndLine = lines.indexOfFirst { 
        ...
        for (i in lineNumber - 1 until lines.size) {
            ...
            if (classMatcher.find()) {
                ...
                if (currentChunk.isNotEmpty() && currentType != null) {
                    addCodeChunk(chunks, currentChunk.toString(), currentType, currentName, startLine, lineNumber - 1, currentParent, tokenizer, maxChunkSize, overlapSize)
                }
            ...
            // Check for function declaration
                ...
                if (indentation > 0) {
                    ...
                    currentParent = chunks.lastOrNull { 
                        it.metadata["type"] == "class" || it.metadata["type"] == "interface" || 
                        it.metadata["type"] == "object" || it.metadata["type"] == "enum class" 
    ...
    private fun chunkGenericCode(
        ...
        for (line in lines) {
            ...
            // Check for method declaration
                ...
                currentParent = chunks.lastOrNull { 
                    it.metadata["type"] == "class" || it.metadata["type"] == "interface" || 
                    it.metadata["type"] == "enum" || it.metadata["type"] == "struct" || it.metadata["type"] == "type" 
                }?.metadata?.get("name")?.toString()
                startLine = lineNumber
                indentLevel = currentIndent
            }
    ...
    /**
     * Add a code chunk, splitting if necessary based on token count
     */
    private fun addCodeChunk(
        ...
        for (i in lines.indices) {
            val line = lines[i]
            val lineTokenCount = tokenizer.countTokens(line)
            
            // If adding this line would exceed the max chunk size, create a new chunk
            if (currentTokenCount + lineTokenCount > maxChunkSize && currentChunk.isNotEmpty()) {
                chunks.add(Chunk(
                ...
                val overlapLines = getOverlapLines(currentChunk.toString(), tokenizer, overlapSize)
                currentChunk = StringBuilder(overlapLines)
                currentTokenCount = tokenizer.countTokens(overlapLines)
                chunkStartLine = startLine + i - overlapLines.count { it == '\n' }
                chunkIndex++
            }
            
            // Add line to current chunk
            currentChunk.append(line).append("\n")
            currentTokenCount += lineTokenCount + 1 // +1 for the newline
        }
        ...
        if (currentChunk.isNotEmpty()) {
            chunks.add(Chunk(
                ...
                metadata = mapOf(
                    "type" to type,
                    "name" to if (chunkIndex > 0) "${name ?: "Unknown"}_part$chunkIndex" else (name ?: "Unknown"),
                    "start_line" to chunkStartLine,
                    "end_line" to endLine,
                    "parent_name" to (parentName ?: ""),
    ...
    private fun getOverlapLines(
        ...
        for (i in lines.size - 1 downTo 0) {
            ...
            if (overlapTokenCount + lineTokenCount <= overlapSize || overlapBuilder.isEmpty()) {
                overlapBuilder.insert(0, line + "\n")
                overlapTokenCount += lineTokenCount
            } else {
                break
            }
        }

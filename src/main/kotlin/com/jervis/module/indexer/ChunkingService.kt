...
class ChunkingService(
     ...
     */
    fun createCodeChunks(content: String, language: String): List<CodeChunk> {
        logger.debug { "Creating code chunks for language: $language" }

        // Use the new chunking strategy
        val strategy = chunkStrategyFactory.getStrategy(language)
        val metadata = mapOf("language" to language)
        val chunks = strategy.splitContent(content, metadata, codeMaxChunkSize, codeOverlapSize)
     ...
     */
    fun createTextChunks(content: String, format: String): List<TextChunk> {
            ...
            // Check for function declaration
                ...
                if (indentation > 0) {
                    ...
                    currentParent = chunks.lastOrNull { 
                        it.type == "class" || it.type == "interface" || 
                        it.type == "object" || it.type == "enum class" 
                    }?.name
                    currentType = "method"
                } else {
                    currentParent = null
     ...
     * @return A list of code chunks
     */
    fun chunkGenericCode(content: String, language: String): List<CodeChunk> {
        ...
        for (line in lines) {
            ...
            // Check for method declaration
                ...
                currentParent = chunks.lastOrNull { 
                    it.type == "class" || it.type == "interface" || 
                    it.type == "enum" || it.type == "struct" || it.type == "type" 
                }?.name
                startLine = lineNumber
                indentLevel = currentIndent
            }

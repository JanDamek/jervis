...
class IndexingMonitorWindow(
    ...
    private fun setupEventHandling() {
        ...
        projectTree.addTreeSelectionListener(object : TreeSelectionListener {
    ...
    private fun addStepNode(parentNode: DefaultMutableTreeNode, step: IndexingStep) {
        ...
        for (i in 0 until path.pathCount) {
            ...
            node?.let {
                when (val userData = it.userObject) {
                    ...
                    is StepNodeData -> pathComponents.add("step:${userData.step.id}")
    ...
    private fun findTreePathByIdentifier(pathId: String): TreePath? {
        ...
        for (i in 1 until components.size) {
            ...
            for (j in 0 until currentNode.childCount) {
                ...
                val childId = when (val userData = child.userObject) {
                    is ProjectNodeData -> "project:${userData.state.projectName}"
                    is StepNodeData -> "step:${userData.step.id}"
                    else -> "unknown"
                }
    ...
    private fun updateLogArea(node: DefaultMutableTreeNode) {
        when (val userData = node.userObject) {
            is ProjectNodeData -> {
                ...
                logs.add("Started: ${state.startTime?.let { formatter.format(it.atZone(java.time.ZoneId.systemDefault())) } ?: "Not started"}")
                state.endTime?.let { 
                ...
                state.overallProgress?.let { progress ->
                    logs.add("Progress: ${progress.current}/${progress.total} (${String.format("%.1f", progress.percentage)}%)")
                }
                logs.add("")
                logs.add("=== STEPS OVERVIEW ===")
                state.steps.forEach { step ->
                    logs.add("${step.name}: ${step.status}")
                }
            ...
            is StepNodeData -> {
                ...
                val logs = mutableListOf<String>()
                logs.add("=== STEP: ${step.name} ===")
                logs.add("Description: ${step.description}")
    ...
    suspend fun handleProgressEvent(event: IndexingProgressEvent) {
        withContext(Dispatchers.Swing) {
            ...
            logger.debug { "Received progress event: ${event.stepName} - ${event.status}" }
        }
    }
    
    // Data classes for tree nodes
    data class ProjectNodeData(val state: ProjectIndexingState) {
        override fun toString(): String {
            ...
            val progressText = state.overallProgress?.let { 
                " (${it.current}/${it.total})" 
            } ?: ""
    ...
    data class StepNodeData(val step: IndexingStep) {
        override fun toString(): String {
            ...
            val progressText = step.progress?.let { 
                " (${it.current}/${it.total})" 
            } ?: ""
            return "$statusIcon ${step.name}$progressText"
        }
    }
    ...
    private inner class IndexingTreeCellRenderer : DefaultTreeCellRenderer() {
        override fun getTreeCellRendererComponent(
            ...
            when (val userData = node?.userObject) {
                is ProjectNodeData -> {
                    foreground = when (userData.state.status) {
                ...
                is StepNodeData -> {
                    foreground = when (userData.step.status) {
                        IndexingStepStatus.RUNNING -> Color.BLUE
                        IndexingStepStatus.COMPLETED -> Color.GREEN.darker()
                        IndexingStepStatus.FAILED -> Color.RED

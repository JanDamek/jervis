...
import com.jervis.service.indexer.chunking.Chunk
import com.jervis.service.indexer.chunking.ChunkStrategy
        ...
        val testStrategy =
            object : ChunkStrategy {
                override fun splitContent(
                    ...
                    maxChunkSize: Int,
                    overlapSize: Int,
                ): List<Chunk> = listOf(Chunk(content, metadata.mapValues { it.value as Any }))

                override fun canHandle(contentType: String): Boolean = true
            }

        // Create a simple implementation of ChunkStrategyFactory
        val testFactory =
            object : ChunkStrategyFactory(listOf(testStrategy)) {

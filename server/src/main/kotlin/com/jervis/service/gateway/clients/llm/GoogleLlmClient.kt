...
class GoogleLlmClient(
    ...
    override fun callWithStreaming(
        ...
        flow {
            ...
            val contents = listOf(GoogleContent(role = "user", parts = listOf(GooglePart(text = userPrompt))))
            val requestBody = buildRequestBody(systemPrompt, contents, creativityConfig, config)
            ...
            responseFlow.collect { line ->
                if (line.isNotBlank()) {
                    try {
                        val response = json.decodeFromString<GoogleStreamResponse>(line)
                        ...
                        logger.error(e) { "Error parsing Google streaming response: ${e.message}" }
                    }
                }
            }
    ...
    private fun buildRequestBody(
        ...
        val systemInstruction =
            systemPrompt
                ...
                ?.let { GoogleSystemInstruction(parts = listOf(GooglePart(text = it))) }
    ...
    data class GoogleRequest(
        ...
        val systemInstruction: GoogleSystemInstruction? = null,
    )
    ...
    @Serializable
    data class GoogleSystemInstruction(
        val parts: List<GooglePart>,
    )

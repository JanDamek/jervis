...
- Chat tools (two-tier): `backend/service-orchestrator/app/chat/tools.py` (CHAT_INITIAL_TOOLS 10 tools, TOOL_REQUEST_TOOLS meta-tool, ToolCategory enum, TOOL_CATEGORIES 6 categories, TOOL_DOMAINS)
- Chat agentic loop: `backend/service-orchestrator/app/chat/handler_agentic.py` (request_tools handler, drift guard enforcement, SourceTracker lifecycle)
- Fact checker: `backend/service-orchestrator/app/guard/fact_checker.py` (claim extraction, verification against KB + web evidence, FactCheckResult)
- Fact check helper: `backend/service-orchestrator/app/chat/handler_fact_check.py` (run_fact_check wrapper, confidence_badge metadata)
- Source attribution: `backend/service-orchestrator/app/chat/source_attribution.py` (SourceTracker — KB source + web evidence collection per request)
- Drift detection: `backend/service-orchestrator/app/chat/drift.py` (multi-signal: duplicate calls, tool spam, alternating pairs, domain drift, workflow chains)
- OpenRouter catalog: `backend/service-ollama-router/app/openrouter_catalog.py` (FREE queue cascade, model error tracking, auto-disable 3 errors, rate limit pause 60s, auto-recovery 5min)
...
- O365 VLM Scraper: `backend/service-o365-browser-pool/app/screen_scraper.py` (periodic VLM screenshots, adaptive intervals, auto-discovery)
- O365 Scrape Storage: `backend/service-o365-browser-pool/app/scrape_storage.py` (MongoDB: o365_scrape_results, o365_scrape_messages, o365_discovered_resources)
- O365 Kotlin Callback: `backend/service-o365-browser-pool/app/kotlin_callback.py` (POST session state to Kotlin server for MFA/expired notifications)
- O365 Session Callback API: `backend/server/.../rpc/internal/InternalO365SessionRouting.kt` (POST /internal/o365/session-event → USER_TASK + push)
- O365 Scrape Message Entity: `backend/server/.../entity/teams/O365ScrapeMessageDocument.kt` (maps to o365_scrape_messages collection)
- O365 Discovered Resources: `backend/server/.../entity/teams/O365DiscoveredResourceDocument.kt` (persistent channel/chat storage for settings UI)
...
- Chat polling (Teams): `backend/server/.../service/polling/handler/teams/O365PollingHandler.kt` (triple-mode: OAuth2/GraphAPI, Gateway, VLM scrape → MongoDB)

...
SERVICE_NAME="jervis-tts"
DOCKERFILE="backend/service-tts/Dockerfile"
...
docker build --platform linux/amd64 \
  -t "${IMAGE}" \
  -f "${PROJECT_ROOT}/${DOCKERFILE}" "${PROJECT_ROOT}/backend/service-tts"
...
kubectl apply -f "$SCRIPT_DIR/app_tts.yaml" -n "${NAMESPACE}"

# Validate YAML changes propagated to K8s
validate_deployment_spec "jervis-tts" "$NAMESPACE"

kubectl rollout restart deployment/jervis-tts -n ${NAMESPACE}
kubectl rollout status deployment/jervis-tts -n ${NAMESPACE}
